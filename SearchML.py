...
def Visu(y_test,y_pred_test,model_name):
    ...
    if isinstance(y_test, pd.Series) and isinstance(y_test.index, pd.DatetimeIndex):
        ...
        plot_filename_ts = f'{model_name}_timeseries_predictions_test_set.png'
...
# df.ta.rsi(close=predict, length=14, append=True)
# # Opóźnienie o 1 dzień
# df['RSI_14'] = df['RSI_14'].shift(1)


# # 2. Moving Average Convergence Divergence (MACD)
# # pandas_ta doda kolumny: 'MACD_12_26_9', 'MACDH_12_26_9', 'MACDS_12_26_9'
# df.ta.macd(close=predict, fast=12, slow=26, signal=9, append=True)
# # Opóźnienie o 1 dzień dla każdej z kolumn MACD
# df['MACD_12_26_9'] = df['MACD_12_26_9'].shift(1)
# df['MACDH_12_26_9'] = df['MACDH_12_26_9'].shift(1)
# df['MACDS_12_26_9'] = df['MACDS_12_26_9'].shift(1)


# 3. Stochastic Oscillator (Stoch)
# pandas_ta doda kolumny: 'STOCHk_14_3_3' (%K), 'STOCHd_14_3_3' (%D)
# Poprawka: zamiast 'predict', pandas_ta oczekuje 'close' dla stoch.
# Jeśli 'topredict' to 'Close', to jest OK.
# 1. Prosta Średnia Ruchoma (SMA)
df['SMA_5'] = df['Volume'].rolling(window=5).mean().shift(1)# SMA 10 dni


df['SMA_10'] = df['Volume'].rolling(window=10).mean().shift(1)
# SMA 20 dni
df['SMA_20'] = df['Volume'].rolling(window=20).mean().shift(1)
df['SMA_30'] = df['Volume'].rolling(window=30).mean().shift(1)

# 2. Wykładnicza Średnia Ruchoma (EMA)
df['EMA_5'] = df['Volume'].ewm(span=5, adjust=False).mean().shift(1)
# EMA 20 dni
df['EMA_10'] = df['Volume'].ewm(span=10, adjust=False).mean().shift(1)
# EMA 20 dni
df['EMA_20'] = df['Volume'].ewm(span=20, adjust=False).mean().shift(1)
...
results_df_sorted.to_csv('MLscore.csv')
